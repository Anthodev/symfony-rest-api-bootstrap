...
if [ "$1" = 'php-fpm' ] || [ "$1" = 'php' ] || [ "$1" = 'bin/console' ]; then
	...
	if [ ! -f composer.json ]; then
		...
		symfony new tmp --version="$SYMFONY_VERSION"
	...
	if grep -q ^DATABASE_URL= .env; then
	 ...
	 setfacl -R -m u:www-data:rwX -m u:"$USER_ID":rwX var
	 setfacl -dR -m u:www-data:rwX -m u:"$USER_ID":rwX var
fi

chown -R $USER_ID:$GROUP_ID /srv/app
